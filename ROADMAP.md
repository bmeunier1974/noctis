...
*Theme: the agent already journals every trial to `workspace/state/experiments/`; make that
journal do more work.*
...
- **Cost-profile hardening.** Keep the $0 local-backend path first-class: tighten token budgets,
  context-window fit on small local models, and per-session spend accounting.
